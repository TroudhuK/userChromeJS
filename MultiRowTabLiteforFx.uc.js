...
// @author         Alice0775
// @version        2025/11/24 00:00 Firefox 145 TroudhuK
...
function zzzz_MultiRowTabLite() {
  /* copy of browser/components/tabbrowser/content/drag-and-drop.js because not extendable (all private) with multirow tabs adaptation */
  ...
  window.TabDragAndDrop = class {
    ...
    handle_dragover(event) {
      ...
      var newMargin, newMarginY; // multirow: 2D margin calculation
      /*if (pixelsToScroll) {
      ...
      ind.style.transform = this.#tabbrowserTabs.verticalMode
        ...
        : "translate(" + Math.round(newMargin) + "px," + Math.round(newMarginY) + "px)"; // multirow fix
    }
  ...
  // injection
  delete gBrowser.tabContainer.tabDragAndDrop;
  gBrowser.tabContainer.tabDragAndDrop = new window.TabDragAndDrop(gBrowser.tabContainer);
  gBrowser.tabContainer.tabDragAndDrop.init();

  gBrowser.tabContainer.on_dragstart = function(event) {
    gBrowser.tabContainer.tabDragAndDrop.handle_dragstart(event);
  }
  
  gBrowser.tabContainer.on_dragover = function(event) {
    gBrowser.tabContainer.tabDragAndDrop.handle_dragover(event);
  }
  
  gBrowser.tabContainer.on_drop = function(event) {
    gBrowser.tabContainer.tabDragAndDrop.handle_drop(event);
  }
  ...
  gBrowser.tabContainer.on_dragleave = function(event) {
    gBrowser.tabContainer.tabDragAndDrop.handle_dragleave(event);
  }
}
